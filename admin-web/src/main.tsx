...
function App() {
  ...
  if (session.loading) {
    return <main className="boot"><Loading/><p>Проверяем административный доступ…</p></main>;
  }
    ...
    <div className={`shell${collapsed ? " collapsed" : ""}`}>
      <aside>
        <div className="brand-row">
          ...
          <button className="collapse-button" aria-label="Свернуть меню" onClick={() => setCollapsed((value) => !value)}><Icon name="menu"/></button>
        </div>
        <label className="nav-search"><Icon name="search"/><input ref={searchRef} value={query} onChange={(event: React.ChangeEvent<HTMLInputElement>) => setQuery(event.target.value)} placeholder="Фильтр разделов"/></label>
        <nav>
          {visibleSections.map((item) => <button className={item.id === section ? "active" : ""} key={item.id} onClick={() => navigate(item.id)} title={collapsed ? item.label : undefined}><span className="nav-icon"><Icon name={item.icon}/></span><span className="nav-copy"><strong>{item.label}</strong><small>{item.description}</small></span></button>)}
          {!visibleSections.length && <div className="nav-empty">Ничего не найдено</div>}
        </nav>
      ...
      <main>
        ...
        <div className="mobile-head"><div><Icon name={current?.icon || "dashboard"}/><b>{current?.label}</b></div><button className="mobile-command" aria-label="Глобальный поиск" onClick={() => setPaletteOpen(true)}><Icon name="search"/></button><select value={section} onChange={(event: React.ChangeEvent<HTMLSelectElement>) => navigate(event.target.value as Section)}>{sections.map((item) => <option key={item.id} value={item.id}>{item.label}</option>)}</select></div>
        {views[section]}
      </main>
    </div>
