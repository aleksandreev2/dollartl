    ...
    async def ban_user(message: Message) -> None:
        ...
        if len(parts) < 4:
            await message.answer(
                "Usage: /ban <telegram_id> <6h|1d|7d|30d|permanent> <template or reason>"
            )
            return
        try:
        ...
        await message.answer(
            f"User {target_id} blocked until {escape(expiry)}.\n"
    ...
    async def unban_user(message: Message) -> None:
        ...
        async with SessionFactory() as session:
            ...
            if target is None:
                await message.answer("User is not registered in the bot.")
                return
            count = await service.unban_user(
                target=target,
                admin_telegram_id=settings.admin_telegram_id,
                note=note,
            )
        await message.answer(
            f"Active bans removed: {count}."
            if count
            else "The user has no active ban."
        )
